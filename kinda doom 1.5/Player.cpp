...
void Player::walkWnew(const Walls& wallsTexture, const Walls& wallsCollision)
{
	texture.y += speed * sin(angle * std::numbers::pi / 180);
	texture.x += speed * cos(angle * std::numbers::pi / 180);
	if (checkCollision(wallsCollision))
	{
		texture.y -= speed * sin(angle * std::numbers::pi / 180);
		texture.x -= speed * cos(angle * std::numbers::pi / 180);
		while (!checkCollision(wallsCollision))
		{
...
void Player::walkSnew(const Walls& walls, const Walls& wallsCollision)
{
	texture.y -= speed * sin(angle * std::numbers::pi / 180);
	texture.x -= speed * cos(angle * std::numbers::pi / 180);
	if (checkCollision(wallsCollision))
...
void Player::walkDnew(const Walls& walls, const Walls& wallsCollision)
{
...
void Player::walkAnew(const Walls& walls, const Walls& wallsCollision)
{
	texture.y -= speed * sin((90 + angle) * std::numbers::pi / 180);
	texture.x -= speed * cos((90 + angle) * std::numbers::pi / 180);
	if (checkCollision(wallsCollision))
...
void Player::getPoints(const Walls& walls)
{
	...
	std::cout << "getPoints: " << elapsed2.count() << '\n';
}
